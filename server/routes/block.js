...
router.get('/b/:username', async (req, res) => {
    ...
    db.query(query, [username]).then((row) => { 
        ...
        db.query("INSERT INTO bloquear (iduser, idbloqueado) VALUES ($1, $2)", [blocking, blockedId]).then((row) => {
            deletarSeguindo =`DELETE FROM seguindo WHERE seguindo.idUser = ${blocking} AND seguindo.idSeguido = ${blockerId} OR  seguindo.idUser = ${blockerId} AND seguindo.idSeguido = ${blocking}`;
